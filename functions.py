"""Auto-generated helpers for Linear_regression.ipynb."""
from __future__ import annotations

CELL_SOURCES = {}

CELL_SOURCES[2] = """import numpy as np
import pandas as pd
...
CELL_SOURCES[6] = """# ─── Synthetic CAPM data (used throughout sections 0-2) ──────────────────────
n = 200
...
CELL_SOURCES[12] = """# ─── Illustrative chart: what OLS minimises ───────────────────────────────────
...
CELL_SOURCES[14] = """# ─── OLS from scratch ─────────────────────────────────────────────────────────
...
CELL_SOURCES[18] = """# ─── MLE via numerical optimisation ───────────────────────────────────────────
...
CELL_SOURCES[21] = """# ─── SST / SSR / RSS — geometric decomposition ───────────────────────────────
...
CELL_SOURCES[25] = """# ─── R² and residual diagnostics ──────────────────────────────────────────────
...
CELL_SOURCES[29] = """# ─── Fama-French 2-factor data (MKT + SMB) ───────────────────────────────────
...
CELL_SOURCES[32] = """# ─── Hat matrix, leverage, Cook's distance ────────────────────────────────────
...
CELL_SOURCES[35] = """# ─── Adjusted R² and information criteria ─────────────────────────────────────
...
CELL_SOURCES[38] = """# ─── Multicollinearity and VIF ────────────────────────────────────────────────
np.random.seed(7)
n_mc = 200
...
CELL_SOURCES[41] = """# ─── Omitted Variable Bias simulation ─────────────────────────────────────────
np.random.seed(42)
n_ovb  = 500
...
CELL_SOURCES[45] = """# ─── A4 / A5 / A6 — geometric illustrations ──────────────────────────────────
np.random.seed(42)
n_a = 120
...
CELL_SOURCES[48] = """# ─── Simulate and plot assumption violations ──────────────────────────────────
np.random.seed(0)
n_v = 150
...
CELL_SOURCES[56] = """# ─── Inference from scratch ────────────────────────────────────────────────────
rss_val   = np.sum(residuals**2)
...
CELL_SOURCES[59] = """# ─── GLS from scratch: compare OLS, WLS, GLS on a heteroscedastic DGP ─────────
np.random.seed(42)
n_gls = 200
...
CELL_SOURCES[62] = """# ─── Heteroscedasticity: simulate, detect, fix ────────────────────────────────
np.random.seed(42)
n_h = 250
...
CELL_SOURCES[65] = """# ─── Autocorrelation: simulate, detect, fix ────────────────────────────────────
np.random.seed(42)
n_ar = 300
...
CELL_SOURCES[68] = """# ─── Non-normality: t-distributed errors (heavy tails) ────────────────────────
np.random.seed(42)
n_nn = 300
...
CELL_SOURCES[72] = """# ─── Bias-variance tradeoff visualisation ─────────────────────────────────────
np.random.seed(42)
...
CELL_SOURCES[75] = """# ─── Ridge: from scratch + sklearn + geometric intuition ──────────────────────
np.random.seed(42)
...
CELL_SOURCES[78] = """# ─── Lasso vs Ridge: sparsity and coefficient comparison ──────────────────────
...
CELL_SOURCES[81] = """# ─── Elastic Net: compare Ridge / Lasso / EN on correlated predictors ─────────
np.random.seed(42)
...
CELL_SOURCES[85] = """# ─── Bayesian linear regression: visualise posterior ──────────────────────────
np.random.seed(42)
...
CELL_SOURCES[88] = """# ─── MAP = Ridge: verify analytically ────────────────────────────────────────
np.random.seed(42)
n_map, p_map = 80, 5
...
CELL_SOURCES[91] = """# ─── Full Bayesian regression: posterior and predictive uncertainty ────────────
np.random.seed(42)
...
def run_cell(cell_id, ns=None):
    """Execute a notebook cell source inside the provided namespace and return it."""
    if ns is None:
        ns = {}
    code = CELL_SOURCES[cell_id]
    exec(compile(code, f"<cell {cell_id}>", "exec"), ns)
    return ns
